...
table2_T = [4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000]
table2_sigma = [0.031, 0.27, 2.05, 6.06, 12.0, 19.9, 29.6, 41.1, 54.1, 67.7, 81.5]

def get_tangent(x_data, y_data, i):
    if i == 0:
        return (y_data[1] - y_data[0]) / (x_data[1] - x_data[0])
    if i == len(x_data) - 1:
        return (y_data[-1] - y_data[-2]) / (x_data[-1] - x_data[-2])

    dx1 = x_data[i] - x_data[i - 1]
    dx2 = x_data[i + 1] - x_data[i]
    dy1 = y_data[i] - y_data[i - 1]
    dy2 = y_data[i + 1] - y_data[i]

    m1 = dy1 / dx1
    m2 = dy2 / dx2
    return (m1 * dx2 + m2 * dx1) / (dx1 + dx2)
...
def interpolate(x, x_data, y_data):
    ...
    for i in range(len(x_data) - 1):
        if x_data[i] <= x <= x_data[i + 1]:
            ...
            m0 = get_tangent(x_data, y_data, i)
            m1 = get_tangent(x_data, y_data, i + 1)
...
def get_Rp_and_T0(I):
    ...
    Rp = l_p / (2 * math.pi * (R_rad**2) * integral_val)
    return Rp, T0


def solve_circuit(t_max, h, mode="std"):
    t = 0.0
    I = I_0
    U = U_0
    ...
    print(f"\nЗапуск симуляции '{mode}' (шаг: {h:.1e} с, шагов: {steps})...")
...
def solve_circuit_back(t_final, I_final, U_final, h, steps):
    t_back = t_final
    ...
    for _ in range(steps):
        ...
        k2_I = dI_dt_back(I_back + h_back / 2 * k1_I, U_back + h_back / 2 * k1_U)
        k2_U = dU_dt_back(I_back + h_back / 2 * k1_I)

        k3_I = dI_dt_back(I_back + h_back / 2 * k2_I, U_back + h_back / 2 * k2_U)
    ...
    return t_rev_vals, I_rev_vals, U_rev_vals

def run_all_tasks():
    h_std = 1e-7
    t_max_std = 1000e-6
    t_std, I_std, U_std, Rp_std, T0_std = solve_circuit(t_max_std, h_std, mode="std")

    print(f"Левый край:\tНапряжение: {U_std[0]:.3f} В, Ток: {I_std[0]:.3f} А")
    print(f"Правый край:\tНапряжение: {U_std[-1]:.3f} В, Ток: {I_std[-1]:.3f} А, время: {t_std[-1] * 1e6:.1f} мкс")

    t_rev_vals, I_rev_vals, U_rev_vals = solve_circuit_back(t_final=t_std[-1], I_final=I_std[-1], U_final=U_std[-1], h=h_std, steps=len(t_std)-1)

    print(f"\nСтартовый ток (I0)       = {I_0:.3f} А")
    print(f"Ток после обратного хода = {I_rev_vals[-1]:.3f} А")
    print(f"Абсолютная ошибка        = {abs(I_0 - I_rev_vals[-1]):.3f} А")
    print(f"Стартовое напряжение (U0)       = {U_0:.3f} В")
    print(f"Напряжение после обратного хода = {U_rev_vals[-1]:.3f} В")
    print(f"Абсолютная ошибка               = {abs(U_0 - U_rev_vals[-1]):.3f} В")
    ...
    if t1 and t2:
        t_imp = (t2 - t1) * 1e6
        print(f"\nI_max = {I_max:.1f} А")
        print(f"0.35 * I_max = {threshold:.1f} А")
        print(f"Длительность импульса t_imp = {t_imp:.1f} мкс")

    t_std_mks = [t * 1e6 for t in t_std]
    plt.figure(figsize=(10, 5))
    plt.plot(t_std_mks, I_std, "b-", label="Прямой ход", linewidth=4, alpha=0.5)
    plt.plot([t * 1e6 for t in t_rev_vals], I_rev_vals, "r--", label="Обратный ход", linewidth=2)
    plt.title("Прямой и обратный ход")
    plt.xlabel("t, мкс")
    plt.ylabel("I(t), А")
    plt.legend()
    plt.grid(True)
    ...
    fig.suptitle("Графики зависимости величин от времени", fontsize=14)

    axs[0, 0].plot(t_std_mks, I_std, "b")
    axs[0, 0].set_title("I(t), A")
    axs[0, 0].grid()

    axs[0, 1].plot(t_std_mks, U_std, "r")
    axs[0, 1].set_title("U(t), В")
    axs[0, 1].grid()

    axs[1, 0].plot(t_std_mks, Rp_std, "g")
    axs[1, 0].set_title("Rp(t), Ом")
    axs[1, 0].grid()

    axs[1, 1].plot(t_std_mks, IRp_std, "m")
    axs[1, 1].set_title("I(t) * Rp(t), В")
    axs[1, 1].grid()

    axs[2, 0].plot(t_std_mks, T0_std, "k")
    axs[2, 0].set_title("T0(t), K")
    axs[2, 0].set_xlabel("t, мкс")
    axs[2, 0].grid()
    ...
    t_max_zero = 5e-3
    t_zero, I_zero, _, _, _ = solve_circuit(t_max_zero, h_std, mode="0")
    ...
    if len(peaks_t) >= 3:
        ...
        print(f"Период 1-й волны:        {T_first * 1e6:.1f} мкс")
        print(f"Период последней волны:  {T_last * 1e6:.1f} мкс")
        print(f"Разница периодов:        {abs(T_first - T_last) * 1e3:.6f} мкс")

        print(f"Амплитуда 1-го пика:     {I_first:.6f} А")
        print(f"Амплитуда {len(peaks_I)}-го пика:    {I_last:.6f} А")
        print(f"Потеря амплитуды за {len(peaks_I)} циклов: {abs(I_first - I_last):.2e} А")
    # =========================================================

    plt.figure(figsize=(10, 4))
    plt.plot([t * 1e6 for t in t_zero], I_zero, color="orange")
    plt.title("I(t) при Rk + Rp = 0")
    plt.xlabel("t, мкс")
    plt.ylabel("I(t), А")
    plt.grid()
    plt.show()
    ...
    h_fast = 1e-7
    t_max_const = 20e-6
    t_const, I_const, _, _, _ = solve_circuit(t_max_const, h_fast, mode="200")

    plt.figure(figsize=(10, 4))
    plt.plot([t * 1e6 for t in t_const], I_const, color="red")
    plt.title("I(t) при Rk + Rp = 200 Ом")
    plt.xlabel("t, мкс")
    plt.ylabel("I(t), А")
    plt.grid()
    plt.show()
