...
Пусть $\vec{X}$ --- случайная выборка из генеральной совокупности $X$, а $\vec{x} = (x_1, \dots, x_n)$ --- её реализация, где $n$ --- объём данной выборки.
...
Вариационным рядом, построенным по выборке $\vec{x} = (x_1, \dots, x_n)$, называют последовательность $x_{(1)}, \dots, x_{(n)}$.

Максимальное значение выборки $M_{max}$ рассчитывается по формуле (\ref{eq:max}); минимальное $M_{min}$ --- (\ref{eq:min}).
\begin{equation}
	\label{eq:max}
	M_{\max} = x_{(n)} =  \max\{x_1,\dots,x_n\}
\end{equation}
\begin{equation}
	\label{eq:min}
	M_{\min} = x_{(1)} = \min\{x_1,\dots,x_n\}
\end{equation}

Размах выборки $R$ рассчитывается по формуле (\ref{eq:diff}).
\begin{equation}
	\label{eq:diff}
	R = M_{\max} - M_{\min} = x_{(n)} - x_{(1)}
\end{equation}

Оценка $\hat\mu$ математического ожидания M$X$ рассчитывается по формуле выборочного среднего (\ref{eq:mx}), оценка $S^2$ дисперсии D$X$ --- по формуле исправленной выборочной дисперсии (\ref{eq:dx}).
\begin{equation}
	\label{eq:mx}
...
Разделим отрезок $J = [x_{(1)}, x_{(n)}]$ на $m$ равновеликих промежутков по формуле (\ref{eq:j}):
...
\begin{equation}
	f_n(x) =
	\begin{cases}
		\frac{n_i}{n \cdot \Delta}, x \in J_i \\
		0\ \ , x \not\in J                    \\
	\end{cases}
\end{equation}

где $J_i$ --- $i$-ый промежуток статистического ряда,
$n_i$ --- количество элементов выборки, входящих в этот промежуток.
...
\begin{equation}
	F_n(x) = \frac{\mu(x, \vec x)}{n}
\end{equation}

где $\mu(x, \vec x) = \sum_{i=1}^n I\{x_i < x\}$ --- число элементов выборки $\vec x$, которые имеют значение, меньшее $x$, $I\{x_i < x\} = \begin{cases} 1, x_i < x, \\ 0, x_i \geq x, \end{cases}$ --- индикаторная функция.
...
Результаты расчетов для выборки из индивидуального варианта приведены на формулах (\ref{eq:res_max}), (\ref{eq:res_min}), (\ref{eq:res_r}), (\ref{eq:res_mx}), (\ref{eq:res_dx}), (\ref{eq:res_m}).
\begin{equation}
	\label{eq:res_max}
	M_{\max} = 0.010
\end{equation}
